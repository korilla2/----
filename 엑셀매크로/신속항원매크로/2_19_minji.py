...
def btncmd():
    ...
    for i in range(1, ws.max_row+1):
        for j in select:
            if (ws.cell(row=i, column=3).value == j) or (ws.cell(row=i, column=3).value == j[0]+' '+j[1:]) or (ws.cell(row=i, column=3).value == j[0]+j[1]+' '+j[2:]):
                temp.append(j)
                ws[f'I{i}'] = '지급'
    ...
    for k in temp:
        if temp.count(k) > 1:
            ...
            for q in range(1, ws.max_row+1):
                if (ws.cell(row=q, column=3).value == k) or (ws.cell(row=q, column=3).value == k[0]+' '+k[1:]) or (ws.cell(row=q, column=3).value == k[0]+k[1]+' '+k[2:]):
                    ws.cell(row=q, column=3).fill = y_color
    ...
    for k in temp2:
        if temp2.count(k) > 1:
            ...
            for q in range(1, ws.max_row+1):
                if (ws.cell(row=q, column=3).value == k) or (ws.cell(row=q, column=3).value == k[0]+' '+k[1:]) or (ws.cell(row=q, column=3).value == k[0]+k[1]+' '+k[2:]):
                    ws.cell(row=q, column=3).fill = y_color
                    ws.cell(row=q, column=7).fill = y_color
            del temp2[temp2.index(k)]
...
btn_txt = Button(root, text='지급', command=btncmd, padx=5, pady=5)
btn_txt.pack(side='top')

btn_txt = Button(root, text='양성', command=btncmd2, padx=5, pady=5)
btn_txt.pack(side='top')

btn_txt = Button(root, text='지급초기화', command=btncmd3, padx=5, pady=5)
btn_txt.pack(side='top')

btn_txt = Button(root, text='양성초기화', command=btncmd4, padx=5, pady=5)
btn_txt.pack(side='top')
